...
def main():
    ...
    for entry in data:
        random.shuffle(entry) # This scrambles all the data in each entry. The data in the current json file is sorted which is unideal for quicksort.
    
    with open("Exercise2/ex2.5.json", "w") as file:
        json.dump(data, file)   # This dumps the scrambled data into a new json file.
    
    with open("Exercise2/ex2.5.json", "r") as file:
        dataScrambled = json.load(file)
    
    for entry in dataScrambled:
        end_length = len(entry)
        elapsed_time = timeit.timeit(lambda : func1(entry, 0, end_length - 1), number = 1)
    ...
    with open("Exercise2/sorted.json", "w") as file:
        json.dump(data, file)
